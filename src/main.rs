...
impl Matcher {
    ...
    fn matches(&self, value: &str) -> bool {
        ...
        match self {
            Self::Equals(pattern) => lower_value == pattern,
            Self::Contains(pattern) => lower_value.contains(pattern),
            Self::Prefix(pattern) => lower_value.starts_with(pattern),
            Self::Suffix(pattern) => lower_value.ends_with(pattern),
...
struct Args {
    /// class to focus (shorthand for `--match class:contains=...`)
    #[argh(option, short = 'c')]
    class: Option<String>,

    /// title to focus (shorthand for `--match title:contains=...`)
    #[argh(option, short = 't')]
    title: Option<String>,
